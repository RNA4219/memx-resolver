...
memx は、LLM エージェント向けのローカルファーストな知識/記憶基盤であり、v1 では「投入（ingest）・検索（search）・参照（show）」を API 正本契約に基づき提供することが中核責務である。4層ストア（short/chronicle/memopedia/archive）を前提に、GC（Observer/Reflector）と archive 補償フローは v1.x の実験機能として段階導入する。
...
- 4DB 境界（short/chronicle/memopedia/archive）と lineage 記録前提。
...
6. **Distillation Pipeline (Observer/Reflector)**
   - 責務: shortクラスタ観測→chronicle化→memopedia反映。
...
GCトリガ判定（short_meta + memory_policy）→ Observerで短期ノート抽出・クラスタ化・観測ノート生成（chronicle）→ lineage `observed` 記録 → Reflectorでテーマ統合（memopedia更新/作成）→ lineage `reflected` 記録。
...
### storage layers
- `short.db`, `chronicle.db`, `memopedia.db`, `archive.db`。
- `lineage` による遷移証跡。
