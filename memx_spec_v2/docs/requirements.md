...
- CLI + SQLite ベースで、言語や実行環境に強く依存しない「知識 OS の下層」を構成する。

### 非ゴール

- Web UI やサーバー常駐プロセスの提供。
...
- Go 側では `go/gatekeeper.go` のインターフェースで表現し、`Conn` 経由で利用する。
...
役割：生テキストから short ノートを作成し、`short.db` に保存する。
...
1. `file` or stdin から本文を読み込む。
2. Gatekeeper（kind=`memory_store`）で保存可否を確認（必要なら）。
3. `--no-llm` でなければ MiniLLM を使って：
   - タグ候補生成
   - `relevance / quality / novelty / importance_static` の初期値推定
   - `sensitivity` の推定
4. `tags` / `note_tags` / `notes` / `note_embeddings` / `notes_fts` を更新。
5. `short_meta` を近似的に更新（note_count/token_sum を +1 など）。
6. GC トリガ閾値を超えそうなら `mem gc short` 実行を提案するか、別プロセスで起動する。
...
Go 側では `go/llm_client.go` に interface を定義し、`db.Conn` にこれらをフィールドとして注入して使う。
...
- 言語：Go（単一バイナリビルドを前提）。
- 依存：
  - `github.com/mattn/go-sqlite3` 等の SQLite ドライバ。
  - `spf13/cobra` などの CLI ライブラリ。
- セキュリティ：
