...
#### 2-7-2. 実行主体・承認要否・監査ログ必須項目（集約表）

| 対象節 | operation | 実行主体（actor） | 事前承認 | 監査ログ必須項目 |
| --- | --- | --- | --- | --- |
...
| 2-7-3 | `archive_move` | 自動: `mem gc` 定期ジョブ / 手動: Repo Maintainer または委任運用担当 | 手動時は Repo Maintainer の実行責任を必須 | `op`, `src_note_id`, `dest_note_id`, `sensitivity`, `executor`, `executed_at`, `result` |
| 2-7-4 | `archive_purge` | 自動: Maintainer 設定の定期ジョブ / 手動: Repo Maintainer（必要時 Security Champion へ事後報告） | 手動実行時のみ必須（Repo Maintainer） | `op`, `note_id`, `sensitivity`, `retention_expired_at`, `executor`, `executed_at`, `result` |

失敗時の必須記録は REQ-SEC-AUD 系で定義する。

- `REQ-SEC-AUD-001` (`archive_move` 失敗): `reason`, `retryable`, `next_attempt_at`, `owner` を必須記録。
- `REQ-SEC-AUD-002` (`archive_purge` 失敗): `reason`, `retryable`, `next_attempt_at`, `owner` を必須記録。
